...
def __():
    ...
    return Catalog, Generator, List, Optional, ParquetDataset, Path, PydalaTable, Any, Dict, ProcessPoolExecutor, ThreadPoolExecutor, datetime, os, pa, pd, psutil, pds, tempdir, timedelta, time, mp, np
...
@app.cell
def __(create_large_dataset, measure_memory_usage):
...
@app.cell
def __(tempfile, Path, df, ParquetDataset, measure_memory_usage):
    ...
    return baseline_memory, ds, memory_after_create, temp_dir, temp_path


@app.cell
def __(ds, measure_memory_usage, time):
    ...
    return end_time, memory_pruned, selected_columns, start_time, table


@app.cell
def __(df, measure_memory_usage, memory_after_create, np, pd):
    ...
    return df_optimized, memory_optimized


@app.cell
def __(create_large_dataset, ParquetDataset, temp_path):
    ...
    return df, ds, end_time, high_value, north_region, non_partition_time, partition_time, start_time, table
...
def __(table, time):
    ...
    return complex_filter, complex_time, end_time, start_time


@app.cell
def __(pa, table, time):
    # Efficient scanning
    ...
    return batch, batch_count, scanner, scanner_time, total_rows
...
def __(table, time):
    ...
    return all_columns_time, few_columns, many_columns, projection_time
...
@app.cell
def __(create_large_dataset, ParquetDataset, temp_path, mp):
...
def __(df, process_chunk, time):
    ...
    return chunk_size, sequential_df, sequential_results, sequential_time, start_time


@app.cell
def __(ThreadPoolExecutor, df, process_chunk, time):
    ...
    return chunks, executor, parallel_df, parallel_results, parallel_time
...
def __(ds, pa, time):
    ...
    return batch_df, batch_results, batch_time, scanner, start_time, table


@app.cell
def __(create_large_dataset, ParquetDataset, measure_memory_usage, temp_path):
    # Create dataset for caching examples
...
def __(ds, time):
    ...
    return end_time, queries, result, start_time, table, times_no_cache
...
def __(ds, np, time):
    ...
    return cached_table, end_time, result, start_time, times_with_cache


@app.cell
def __(cached_table, time):
    # Selective caching
    ...
    return end_time, full_table_result, north_data, north_premium, premium_data, start_time


@app.cell
def __(ParquetDataset, create_large_dataset, temp_path):
    ...
    return datasets, df, total_records
...
def __(pa, time):
    ...
    return all_results, final_result, high_value, i, start_time, streaming_time, table
...
def __(pd, time):
    ...
    return aggregated_results, chunk_agg, df_chunk, final_agg, aggregation_time, start_time, table
...
def __(pa, time):
    ...
    return combined_result, lazy_results, lazy_time, scanner, start_time


@app.cell
def __(ParquetDataset, create_large_dataset, measure_memory_usage, temp_path):
...
def __(ds, measure_memory_usage, time):
    ...
    return end_time, mem_after, mem_before, performance_results, query_func, query_name, query_time, result, start_time, table, test_queries


@app.cell
def __(pd, performance_results):
    # Performance summary
...
def __(perf_df):
    ...
    for rec in recommendations:
        print(f"  {rec}")
    return avg_memory, avg_time, recommendations
