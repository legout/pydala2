...
def __():
    ...
    return Any, Catalog, ClientError, ClientError, FileSystem, List, NoCredentialsError, Optional, ParquetDataset, Path, PydalaTable, Union, boto3, datetime, os, pa, pd, pq, pds, tempdir, timedelta, time


@app.cell
def __(np, pd, timedelta, datetime):
    def create_sample_cloud_data():
        ...
        return df
    return create_sample_cloud_data,


@app.cell
def __(boto3, ClientError, NoCredentialsError):
    def check_s3_credentials():
        ...
        try:
            ...
            return False
    return check_s3_credentials,


@app.cell
def __(check_s3_credentials, create_sample_cloud_data):
    ...
    df.head()
    return df, s3_available


@app.cell
def __(ParquetDataset, create_sample_cloud_data, tempfile, Path):
    # Create mock S3-like structure for demonstration
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        mock_ds = ds
    return bucket_path, ds, mock_bucket_path, mock_ds, temp_dir, temp_path


@app.cell
def __(Path, tempfile, df, ParquetDataset):
    # Create temporary directory for S3 simulation
    ...
    print(f" - Shape: {len(table)} rows, {len(table.columns)} columns")
    return bucket_path, ds, table, temp_dir, temp_path
...
def __(os, table):
    ...
    for i, file_path in enumerate(ds.files[:3]):
        ...
        print(f"  {i+1}. {s3_path} ({size:.1f} KB)")
    return active_records, file_path, i, size, s3_path, us_east_records


@app.cell
def __(ParquetDataset, create_sample_cloud_data, temp_path):
    ...
    print(f" - Selected {len(selected_columns.columns)} of {len(table.columns)} columns")
    return df, ds_optimized, end_time, selected_columns, start_time, table
...
def __(table, time):
    ...
    return end_time, filtered_data, start_time
...
def __(pa, table, time):
    ...
    return batch, batch_count, scanner, total_rows


@app.cell
def __(ParquetDataset, create_sample_cloud_data, temp_path):
    # Create multiple datasets to simulate remote access
    datasets = []
    for i in range(3):
        df = create_sample_cloud_data()
        df['batch'] = f"batch_{i}"
    ...
    print(f"\n3.1 Created {len(datasets)} remote datasets")
    return datasets, df, i
...
def __(pa, time):
    ...
    print(f" - Combined records: {combined_data.num_rows}")
    return all_data, combined_data, data, sequential_time, start_time, table
...
def __(pa, time):
    ...
    if efficient_results:
        ...
        print(f" - Efficient records: {efficient_data.num_rows}")
    return batch, efficient_data, efficient_results, efficient_time, scanner, start_time
...
def __(pa, time):
    ...
    print(f" - Cached result: {len(cached_result)} records")
    return cached_result, cached_us_east, cache_time, east_data, start_time, us_east_data


@app.cell
def __(ParquetDataset, create_sample_cloud_data, temp_path):
    # Create test datasets with different configurations
    configurations = [
    ...
    for config in configurations:
        ...
        print(f"   - Aggregation: {time3:.3f}s")
    return config, configurations, df, df_agg, result, result1, result2, results, start_time, table, time1, time2, time3
...
def __(results):
    ...
    print("   - Use compression to reduce storage costs")
    return r,
...
def __():
    ...
    safe_s3_operation("Read S3 object", simulate_error=True)
    return ConnectionError, error, error_examples, operation_name, safe_s3_operation, simulate_error
...
def __():
    ...
    print("   - Set up alerts for unusual error patterns")


@app.cell
def __(shutil, s3_available, temp_dir):
    import shutil
    ...
    if not s3_available:
        ...
        print("For real S3 access, configure AWS credentials.")
    return shutil,
