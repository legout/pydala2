...
from fsspec_utils.core import MonitoredSimpleCacheFileSystem, DirFileSystem, filesystem, AbstractFileSystem
#from fsspec_utils import filesystem as fsspec_utils_filesystem, MonitoringFileSystem,
...
def FileSystem(**kwargs) -> AbstractFileSystem:
    """Create a filesystem instance using fsspec-utils with pydala enhancements."""
    # Handle protocol_or_path parameter
    if "protocol_or_path" in kwargs:
        protocol_or_path = kwargs.pop("protocol_or_path")
    else:
        protocol = kwargs.pop("protocol", "file")
        path = kwargs.pop("path", "")
        protocol_or_path = f"{protocol}://{path}"

    return filesystem(protocol_or_path=protocol_or_path, **kwargs)
...
def PyArrowFileSystem(
    ...
    **kwargs
    ...
    if path is not None:
        ...
        fs = pfs.SubTreeFileSystem(base_fs=fs, base_path=path)
